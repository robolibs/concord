...
namespace concord {
    namespace indexing {
        ...
        namespace spatial_indexing {
             ...
             */
            template <typename T> class PointIndex {
                QuadTree<T> tree_;
              ...
              public:
                ...
                std::vector<typename QuadTree<T>::Entry> queryRect(const AABB &rect) const { return tree_.query(rect); }
             ...
             */
            template <typename T> class FastPointIndex {
              ...
              public:
                ...
                std::vector<T> queryRect(const AABB &rect) const { return grid_.queryRect(rect); }
             ...
             */
            template <typename T> class GeometryIndex {
                RTree<T> tree_;

              public:
                GeometryIndex(size_t max_entries = 16, size_t min_entries = 4) : tree_(max_entries, min_entries) {}
                ...
                std::vector<typename RTree<T>::Entry> queryRect(const AABB &rect) const { return tree_.search(rect); }
             ...
             */
            template <typename T> PointIndex<T> createPointIndex(const AABB &boundary) {
                return PointIndex<T>(boundary);
            }

            template <typename T> FastPointIndex<T> createFastPointIndex(double cell_size) {
            ...
            namespace utils {
                 ...
                 */
                inline double recommendCellSize(const std::vector<Point> &points,
                                                double target_points_per_cell = 10.0) {
                    if (points.empty())
                        return 1.0;
                 ...
                 */
                template <typename Container> bool shouldUsePointIndex(const Container &geometries) {
