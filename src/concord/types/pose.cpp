#include "concord/types/pose.hpp"
#include <array>
#include <cmath>

namespace concord {

    Pose::Pose(const Point &p, const Euler &a) : point(p), angle(a) {}

    Pose::Pose(float x, float y, float yaw) : point(Point{x, y, 0.0f}), angle(Euler{0.0f, 0.0f, yaw}) {}

    Pose::Pose(const Point &p, const Quaternion &q) noexcept : point(p), angle(q) {}

    bool Pose::is_set() const { return point.is_set() && angle.is_set(); }

    Point Pose::transform_point(const Point &local_point) const {
    ...
    Point Pose::inverse_transform_point(const Point &world_point) const {
    ...
    bool Pose::operator==(const Pose &other) const { return point == other.point && angle == other.angle; }

    bool Pose::operator!=(const Pose &other) const { return !(*this == other); }

    std::vector<Point> Pose::get_corners(Size size) const {
