...
namespace concord {
    ...
    Partitioner::Partitioner(const Polygon &poly) : border_(poly) {}

    Partitioner::PartitionCriteria::PartitionCriteria()
        : max_area(50000.0), min_convexity(0.6), max_aspect_ratio(4.0), min_bridge_width(20.0), tooth_threshold(0.3),
          simplify_tolerance(2.0), max_recursion_depth(5), enable_bridge_detection(true), enable_tooth_detection(true),
          enable_aspect_splitting(true) {}

    // Main partitioning function with intelligent multi-criteria splitting
    std::vector<Polygon> Partitioner::partition(double area_threshold, const PartitionCriteria &criteria) {
        criteria_ = criteria;
    ...
    // Partition into approximately equal areas
    std::vector<Polygon> Partitioner::partition_equal_areas(double target_area, double tolerance) {
        polygons_.clear();
        std::vector<Polygon> result;
        ...
        if (num_parts <= 4) {
            std::cout << "Using area-based equal splitting for " << num_parts << " parts" << std::endl;
            // Use area-based splitting which is more predictable
            grid_parts = split_by_area_equal(border_, num_parts);
            std::cout << "Area-based splitting returned " << grid_parts.size() << " parts" << std::endl;
        } else {
            std::cout << "Using grid splitting for " << num_parts << " parts" << std::endl;
    ...
    const Partitioner::PartitionCriteria &Partitioner::getCriteria() const { return criteria_; }

    // Set partitioning criteria
    void Partitioner::setCriteria(const PartitionCriteria &criteria) { criteria_ = criteria; }

    // Recursive partitioning with multiple criteria
    void Partitioner::partition_recursive(const Polygon &poly, std::vector<Polygon> &result, int depth) {
    ...
    bool Partitioner::meets_all_criteria(const Polygon &poly, double area) const {
        // Area check
        if (area > criteria_.max_area) {
            return false;
        }
    ...
    double Partitioner::calculate_convexity_ratio(const Polygon &poly) const {
    ...
    double Partitioner::calculate_aspect_ratio(const Polygon &poly) const {
    ...
    std::vector<Polygon> Partitioner::split_by_area(const Polygon &poly) const {
        double area = poly.area();
        if (area <= criteria_.max_area) {
    ...
    std::vector<Polygon> Partitioner::split_vertically(const Polygon &poly) const {
    ...
    std::vector<Polygon> Partitioner::split_horizontally(const Polygon &poly) const {
        const auto &points = poly.getPoints();
        if (points.size() < 3) {
            return {poly}; // Can't split
        }
    ...
    std::vector<Polygon> Partitioner::split_by_narrow_bridges(const Polygon &poly) const {
    ...
    bool Partitioner::is_potential_bridge_cut(const Polygon &poly, const Line &cut) const {
    ...
    std::vector<Polygon> Partitioner::split_by_teeth_and_extensions(const Polygon &poly) const {
        // Get convex hull
        Polygon convex_hull = spatial::convexHull(poly.getPoints());
    ...
    bool Partitioner::is_valid_cutting_line(const Polygon &poly, const Line &cut) const {
    ...
    bool Partitioner::is_tooth_like(const std::vector<Polygon> &parts, double original_area) const {
        if (parts.size() != 2)
            return false;
    ...
    std::vector<Polygon> Partitioner::split_by_aspect_ratio(const Polygon &poly) const {
        Bound obb = poly.get_obb();
    ...
    std::vector<Polygon> Partitioner::split_by_convexity(const Polygon &poly) const {
    ...
    // Split polygon into N approximately equal parts
    std::vector<Polygon> Partitioner::split_by_area_equal(const Polygon &poly, int num_parts) const {
        if (num_parts <= 1) {
            return {poly};
        }
    ...
    std::vector<Polygon> Partitioner::balance_part_areas(const std::vector<Polygon> &parts, double target_area,
        ...
        for (size_t i = 0; i < part_areas.size(); ++i) {
            ...
            // If part is too large, try to split it
            else if (area > target_area * (1 + tolerance)) {
                int subdivisions = std::round(area / target_area);
                if (subdivisions > 1) {
                    // Use grid split for large parts
                    auto sub_parts = split_by_grid(poly, subdivisions);
                    for (const auto &sub : sub_parts) {
                        if (sub.area() > target_area * 0.1) {
                            balanced.push_back(sub);
                        }
                    }
                } else {
                    balanced.push_back(poly);
    ...
    std::vector<Polygon> Partitioner::split_by_grid(const Polygon &poly, int divisions) const {
        if (divisions <= 1) {
            return {poly};
        }
    ...
    std::vector<Polygon> Partitioner::split_polygon_with_line(const Polygon &poly, const Line &cutting_line) const {
